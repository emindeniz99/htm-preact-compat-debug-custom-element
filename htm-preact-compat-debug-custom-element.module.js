...
import htm from "htm";
import { createElement, cloneElement, render, hydrate } from "preact/compat";
...
// modified via replacing default preact h() function with preact/compat createElement() function
...
function toVdom(element, nodeName) {
  ...
  for (i = cn.length; i--; ) {
    ...
    if (name) {
      props[name] = createElement(Slot, { name }, vnode);
    } else {
      children[i] = vnode;
    }
  }
  ...
  const wrappedChildren = nodeName
    ? createElement(Slot, null, children)
    : children;
